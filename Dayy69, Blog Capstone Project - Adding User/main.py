...
class Comments(db.Model):
    ...
    author_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    c_author = relationship('Users', back_populates="comments")
...
def admin_only(function):
    ...
    def decorated(*args, **kwargs):
        if current_user.id == 1 and current_user.is_authenticated:
            ...
            abort(404, description="Resource is not found")
    return decorated
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if not check_email:
            ...
            flash("User is already exist! Please log in")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            new_comment = Comments(text=form.comment.data, c_author=current_user, parent_post=requested_post)
            db.session.add(new_comment)
            db.session.commit()
        else:
            flash("You have to login in")
            return redirect(url_for('login'))
    return render_template("post.html", post=requested_post, comments_=form, current_user=current_user)
